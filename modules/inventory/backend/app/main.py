...
MODULE_NAME     = os.environ.get("MODULE_NAME", "inventory")
REQUIRED_ROLE   = os.environ.get("REQUIRED_ROLE", "inventory")
...
def current_claims(request: Request) -> Claims:
    return decode_and_validate(get_bearer_token(request))

def role_guard(claims: Claims = Depends(current_claims)) -> Claims:
    require_role(claims); return claims

@app.get("/healthz")
def healthz(): return {"ok": True, "module": MODULE_NAME, "port": PORT}

@app.get("/readyz")
def readyz(): return {"ready": True, "module": MODULE_NAME}

@app.get("/api/healthz")
def api_healthz(): return {"ok": True, "scope": "api", "module": MODULE_NAME}
